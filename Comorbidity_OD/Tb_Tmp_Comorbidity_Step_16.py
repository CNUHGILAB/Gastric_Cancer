    ...
    def run(
        ...
        sql = '''
            SELECT
                ...
                CASE
                    WHEN 진료서식구성원소ID = '100095806' THEN replace(의무기록내용, '1', '+')
                    WHEN 진료서식구성원소ID = '100095805' THEN replace(의무기록내용, '1', '-')
                end as CVA
            FROM
                gc_raw.outpatient
            WHERE
                진료서식구성원소ID = '100095806'
                OR 진료서식구성원소ID = '100095805'
        '''
        ...
        for x in o_ID:
            f = open("Comorbidity/CVA.txt",'rt',encoding='UTF8')
            sql= ''
            while True:
                line = f.readline()
                if not line: break
        ...
        print(df2)
        self.insert(df2, db_name="gc_protocol", tb_name="tb_tmp_comorbidity_step_16")
