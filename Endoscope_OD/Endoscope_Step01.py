...
class EndoscopeStep01(BaseETL):
    ...
    def run(
        ...
        for x in o_ID:
            sql = "SELECT 원무접수ID, 환자번호, 검사시행일, 검사결과 FROM gc_raw.endoscope WHERE 검사서식세부항목ID = '{0}'".format(x)
        ...
        self.insert(df, db_name="gc_protocol", tb_name="endoscopestep01") 
