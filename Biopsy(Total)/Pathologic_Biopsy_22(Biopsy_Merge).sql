SELECT
    ...
    pSafe_Margin,
    pTNM_ver,
    #COALESCE(pT, T_stage) AS pT, -- T_stage: Macroscopic Biospy
    pT,
    CASE
        WHEN pN != "None"
        THEN pN
        WHEN metLN = 0
        THEN "N0"
        WHEN (metLN BETWEEN 1 AND 2)
        THEN "N1"
        WHEN (metLN BETWEEN 3 AND 6)
        THEN "N2"
        WHEN (metLN BETWEEN 7 AND 15)
        THEN "N3a"
        WHEN metLN >= 16
        THEN "N3b"
    END AS pN,
