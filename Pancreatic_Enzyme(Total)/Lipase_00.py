        ...
        sql= '''
            ...
            FROM(
                SELECT
                    ...
                    원무접수ID,
                    CASE
                        WHEN 검사코드 = 'B2621E' AND REGEXP_INSTR(검사결과, '<|>|=|(|)') = 0
                        THEN 검사결과
                        WHEN 검사코드 = 'B2621E' AND REGEXP_INSTR(검사결과, '<|>|=|(|)') != 0
                        THEN `검사결과-수치값`
                    END AS Lipase,
            ...
            WHERE Lipase IS NOT NULL
        '''
            
        df = self.df_from_sql(db_name = "pancreatic_enzyme_protocol", sql = sql)
        #df.to_excel('D:/Gastric_Cancer_xlsx/Registry(2012-2022)/Registry_26.xlsx')
        
        self.insert(df, db_name = "pancreatic_enzyme_protocol", tb_name = "lipase_00")
